...
#define ITERS_START = 1;
#define DELTA_START = 1;
#define increment_iterations = 20;
...
main(int argc, char *argv[])
{
  ...
  long int c_iter, c_delta = DELTA;
  long int MAXSIZE = BASE+(ITERS+1)*DELTA;

  printf("\n Hello World -- 2D Combine \n");

  for(c_iter = ITERS_START; c_iter < ITERS; c_iter+= (ITERS- ITERS_START)/increment_iterations){
    MAXSIZE = BASE + (c_iter+1)*c_delta;
    ...
    for (i = 0; i < c_iter; i++) {
      ...
      set_vec_length(v0,BASE+(i+1)*c_delta);
